...
export const assignments = sqliteTable("assignments", {
  ...
  postId:integer("post_id").references(()=>posts.id), vehicleId:integer("vehicle_id").references(()=>vehicles.id), shift:text("shift").notNull(), role:text("role",{enum:["guard","driver","patrol","third"]}).notNull().default("guard"), startsAt:text("starts_at").notNull(), endsAt:text("ends_at").notNull(), regularEndsAt:text("regular_ends_at"), breakStartsAt:text("break_starts_at"), breakEndsAt:text("break_ends_at"), workKind:text("work_kind").notNull().default("shift"), status:text("status",{enum:["normal","overtime","time_bank","swap"]}).notNull().default("normal"), requestRef:text("request_ref"), isReassigned:integer("is_reassigned",{mode:"boolean"}).notNull().default(false), reassignmentNote:text("reassignment_note"), laneOrder:integer("lane_order"), ...audit,
...
export const weeklyReturnContexts = sqliteTable("weekly_return_contexts", {
  guardId:integer("guard_id").primaryKey().references(()=>guards.id),
  patternSlotsJson:text("pattern_slots_json").notNull().default("[]"),
  patternGroupsJson:text("pattern_groups_json").notNull().default("[]"),
  ...audit,
});
...
export const patternSlots = sqliteTable("pattern_slots", {
 id:integer("id").primaryKey({autoIncrement:true}), patternId:integer("pattern_id").notNull().references(()=>shiftPatterns.id), guardId:integer("guard_id").notNull().references(()=>guards.id), postId:integer("post_id").references(()=>posts.id), vehicleId:integer("vehicle_id").references(()=>vehicles.id), shift:text("shift"), role:text("role",{enum:["guard","driver","patrol","third"]}).notNull(), ...audit,
},t=>[uniqueIndex("idx_pattern_slots_pattern_guard").on(t.patternId,t.guardId)]);
export const patternOperationalGroupMembers = sqliteTable("pattern_operational_group_members", {
  id:integer("id").primaryKey({autoIncrement:true}), patternId:integer("pattern_id").notNull().references(()=>shiftPatterns.id), groupId:integer("group_id").notNull().references(()=>operationalGroups.id), resourceKind:text("resource_kind").notNull(), resourceId:integer("resource_id").notNull(), teamLabel:text("team_label"), shift:text("shift"), vehicleId:integer("vehicle_id").references(()=>vehicles.id), startsAt:text("starts_at"), endsAt:text("ends_at"), ...audit,
},t=>[uniqueIndex("idx_pattern_operational_group_member_unique").on(t.patternId,t.resourceKind,t.resourceId),index("idx_pattern_operational_group_member_pattern").on(t.patternId,t.resourceKind,t.resourceId)]);
