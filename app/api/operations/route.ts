...
async function loadOperations(date:string){
  ...
  const [operations,vehicles,slots]=await Promise.all([
    ...
    env.DB.prepare(`SELECT os.*,g.name guard_name,g.registration,g.platoon,ov.vehicle_id,v.prefix vehicle_prefix,COALESCE(p.name,v0.prefix,'À disposição') origin_label
      ...
      LEFT JOIN vehicles v0 ON v0.id=a.vehicle_id
...
async function getSuggestions(operationId:number,slotId:number){
  const slot=await env.DB.prepare(`SELECT os.*,o.schedule_id,o.starts_at operation_starts_at,o.ends_at operation_ends_at,o.status operation_status,s.date
    FROM operation_slots os JOIN operations o ON o.id=os.operation_id JOIN schedules s ON s.id=o.schedule_id
    WHERE os.id=? AND os.operation_id=? AND o.status!='cancelled'`).bind(slotId,operationId).first<Row>();
  if(!slot)return null;
  ...
  const [guards,movements,assignments,operationGuards,he]=await Promise.all([
    ...
    env.DB.prepare(`SELECT a.*,COALESCE(p.name,v.prefix,'À disposição') origin_label,
      ...
      FROM assignments a LEFT JOIN posts p ON p.id=a.post_id LEFT JOIN vehicles v ON v.id=a.vehicle_id
      WHERE a.schedule_id=?`).bind(date,date,slot.schedule_id).all<Row>(),
  ...
  const items=guards.results.flatMap(guard=>{
    ...
    return [{
      ...
      operationHours:sourceType==="available"?0:operationHours,
    }];
  }).sort((a,b)=>{
    const priority={available:0,extension:1,overtime:2};
...
export async function POST(request:Request){
  ...
  if(body.action==="assign_slot"){
    ...
    await env.DB.prepare("UPDATE operation_slots SET guard_id=?,source_type=?,origin_assignment_id=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND operation_id=?")
  ...
  if(body.action==="clear_slot"){
    ...
    await env.DB.prepare("UPDATE operation_slots SET guard_id=NULL,source_type='pending',origin_assignment_id=NULL,updated_at=CURRENT_TIMESTAMP WHERE id=? AND operation_id=?").bind(slotId,operationId).run();
  ...
  if(body.action==="cancel"){
    ...
    await env.DB.prepare("UPDATE operations SET status='cancelled',updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(operationId).run();
