...
async function ensureCatalog() {
  ...
  if(Number(vehicleCount?.total||0)<4) await env.DB.batch([
    ...
    env.DB.prepare("INSERT OR IGNORE INTO vehicles (prefix,type,zone) VALUES (?,?,?)").bind("VTR 1273","suv","Escola Mais Segura"),
  ]);
}
...
function shiftTimes(date:string, shift:string) {
  ...
  const [start,end]=values[shift]; return {start:`${date}T${start}`,end:`${shift==="4"?tomorrow.toISOString().slice(0,10):date}T${end}`};
}

async function seedSchedule(date:string, scheduleId:number) {
  const existing=(await env.DB.prepare("SELECT guard_id,post_id,vehicle_id,shift,role FROM assignments WHERE schedule_id=?").bind(scheduleId).all<Record<string,unknown>>()).results;
  if(existing.length>=20) return;
  const guards=(await env.DB.prepare("SELECT id FROM guards WHERE active=1 ORDER BY id").all<{id:number}>()).results;
  const posts=(await env.DB.prepare("SELECT id FROM posts WHERE active=1 ORDER BY sort_order,id").all<{id:number}>()).results;
  const vehicles=(await env.DB.prepare("SELECT id FROM vehicles WHERE active=1 ORDER BY prefix").all<{id:number}>()).results;
  const slots=[...posts.map(p=>({postId:p.id,vehicleId:null,role:"guard"})),...vehicles.flatMap(v=>[{postId:null,vehicleId:v.id,role:"driver"},{postId:null,vehicleId:v.id,role:"patrol"}])];
  const commands:D1PreparedStatement[]=[];
  for(const period of [["2","3"],["4","1"]]) {
    const used=new Set(existing.filter(a=>period.includes(String(a.shift))).map(a=>Number(a.guard_id)));
    for(const slot of slots) {
      const missing=period.filter(shift=>!existing.some(a=>String(a.shift)===shift&&Number(a.post_id||0)===Number(slot.postId||0)&&Number(a.vehicle_id||0)===Number(slot.vehicleId||0)&&String(a.role)===slot.role));
      if(!missing.length) continue;
      const guard=guards.find(g=>!used.has(g.id)); if(!guard) break; used.add(guard.id);
      for(const shift of missing) {const time=shiftTimes(date,shift);commands.push(env.DB.prepare("INSERT OR IGNORE INTO assignments (schedule_id,guard_id,post_id,vehicle_id,shift,role,starts_at,ends_at,status) VALUES (?,?,?,?,?,?,?,?,?)").bind(scheduleId,guard.id,slot.postId,slot.vehicleId,shift,slot.role,time.start,time.end,"normal"));}
    }
  }
...
async function ensureBase(date:string) {
  ...
  if(schedule) await seedSchedule(date,schedule.id);
}
