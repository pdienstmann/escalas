...
function priorityScore(day: PlanningDay) {
  const periods = [day.day, day.night];
  return periods.reduce((score, period) => score + Number(period.holes || 0) * 12 + Number(period.away || 0) * 3 + (period.status === "attention" ? 2 : 0), 0);
}
...
export function MonthlyPlanning({ initialDate }: { initialDate: string }) {
  ...
  const [resourceFilter, setResourceFilter] = useState<"all" | "vehicle" | "post" | "group">("all");
  ...
  const priorityDays = useMemo(() => (data?.days || []).filter((day) => day.status !== "ok" || day.day.away > 0 || day.night.away > 0).sort((a, b) => priorityScore(b) - priorityScore(a) || a.date.localeCompare(b.date)).slice(0, 6), [data]);
...
function PlanningPeriodChip({ label, period }: { label: string; period: Period }) {
  ...
  return <div className={`planning-period-chip ${period.status} ${label === "Diurno" ? "period-day" : "period-night"}`}><header><b>{label}</b><span>{period.code}</span></header><strong>{period.available}<small> GMs disponíveis</small></strong><p><b>{leaves}</b> folga{leaves === 1 ? "" : "s"}{period.holes ? ` · ${period.holes} furo${period.holes === 1 ? "" : "s"}` : ""}</p></div>;
}
