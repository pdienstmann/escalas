...
export function OvertimeDashboard() {
  ...
  const [activeView,setActiveView]=useState<"review"|"distribution">("review");
  ...
  return <main className="overtime-page">
    ...
    <nav className="he-view-tabs" aria-label="Visualizações de horas extras"><button type="button" className={activeView==="review"?"active":""} onClick={()=>setActiveView("review")}><b>Conferir serviços</b><span>{pendingCount} pendentes</span></button><button type="button" className={activeView==="distribution"?"active":""} onClick={()=>setActiveView("distribution")}><b>Distribuição por GM</b><span>Ranking, filtros e histórico</span></button></nav>
    <section className={`he-panel he-ledger ${activeView!=="review"?"he-view-hidden":""}`}><div className="he-head"><div><h2>Conferência dos serviços</h2><p>Confira o que foi efetivamente realizado antes de contabilizar.</p></div><div className="he-filters"><select value={entryFilter} onChange={(event)=>setEntryFilter(event.target.value as typeof entryFilter)} aria-label="Filtrar lançamentos"><option value="pending">Pendentes ({pendingCount})</option><option value="reviewed">Já conferidos</option><option value="all">Todos</option></select></div></div>
