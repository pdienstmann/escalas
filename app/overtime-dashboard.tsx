...
export function OvertimeDashboard() {
  ...
  return <main className="overtime-page">
    <header><BackToSchedule date={date}/><div><span>LIVRO MENSAL E DISTRIBUIÇÃO</span><h1>Horas extras</h1><p>Somente horas conferidas entram no total oficial.</p></div><label>Mês<input type="month" value={month} onChange={(event)=>setSelectedMonth(event.target.value)}/></label><button onClick={()=>setManualOpen(true)}>+ Lançar HE</button><button onClick={exportCsv}>Exportar CSV</button></header>
