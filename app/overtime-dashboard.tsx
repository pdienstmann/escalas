...
export function OvertimeDashboard() {
  ...
  const ranking = useMemo(() => {
    ...
    return rows;
  }, [data, eligibility, query, rankingSort, teamFilter]);
  if (!data)
  ...
  return <main className="overtime-page manual-he-page">
    <header><BackToSchedule date={date}/><div><span>CONTROLE MANUAL E DISTRIBUIÇÃO</span><h1>Horas extras</h1><p>O saldo só muda quando a equipe responsável registra ou ajusta as horas.</p></div><span className={`he-month-state ${monthClosed?"closed":"open"}`}>{monthClosed?"Mês fechado":"Mês aberto"}</span><label>Mês<input type="month" value={month} onChange={(event)=>setSelectedMonth(event.target.value)}/></label><button disabled={monthClosed} title={monthClosed?"Reabra o mês para lançar horas extras":""} onClick={()=>setManualOpen("blank")}>+ Lançar HE</button><button onClick={()=>setClosureDialog(monthClosed?"reopen":"close")}>{monthClosed?"Reabrir mês":"Fechar mês"}</button><button onClick={exportCsv}>Exportar CSV</button></header>
